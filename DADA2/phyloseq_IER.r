...
meta <- read.table("/Users/whitneyware/IER_Sleeve/meta/IER_meta.txt", sep = "\t", header = TRUE, row.names = 1)
meta <- as.data.frame(meta)


ps <- phyloseq(otu_table(IER_merged_fr, taxa_are_rows=FALSE), sample_data(meta), tax_table(tax))
...
rm(ps)
tax <- readRDS("/Users/whitneyware/IER_Sleeve/IER_combined/IER_combined_tax_final.rds")
IER_merged_fr <- readRDS("/Users/whitneyware/IER_Sleeve/IER_combined/IER_combined_seqtab_final.rds")